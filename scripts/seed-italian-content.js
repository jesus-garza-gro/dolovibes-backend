...
async function seedItalian() {
    ...
    // 2. Crear experiencias en italiano como TRADUCCIONES (usando PUT con documentId)
    ...
    for (const exp of experiencesEs.data.data) {
        ...
        try {
            ...
            const check = await axios.get(`${STRAPI_URL}/api/experiences`, {
                params: { locale: 'it', 'filters[slug][$eq]': exp.slug },
                headers: authHeaders,
            });
            ...
            await axios.put(
                `${STRAPI_URL}/api/experiences/${exp.documentId}?locale=it`,
                { data: dataToSend },
                { headers: { ...authHeaders, 'Content-Type': 'application/json' } }
            );
