...
async function checkIfGermanPackageExists(slug) {
    try {
...
async function createGermanVersion(pkg, translation) {
    ...
    try {
        ...
        const existing = await checkIfGermanPackageExists(pkg.slug);
        const action = existing ? 'actualizado' : 'creado';
        
        if (existing) {
            console.log(`♻️  Ya existe en alemán: ${translation.title} (actualizando...)`);
        }
        
        const response = await axios.put(
            `${STRAPI_URL}/api/packages/${pkg.documentId}?locale=de`,
            { data: germanData },
            { 
                headers: { 
                    ...authHeaders, 
        ...
        console.log(`✅ ${action.charAt(0).toUpperCase() + action.slice(1)} en alemán: ${translation.title}`);
